	...
	<div class="w3-main" style="margin-right:100px; margin-left:100px; margin-top:100px">
		...
		<tr>
			...
			<td rowspan="10" style="width: 220px;">
				...
				<c:if test="${member.mem_num==host }"><a href="mypage/photoEdit">사진 수정</a></c:if>
			</td>
			<td>이름 ${member.mem_Nickname }</td>
			<td><c:if test="${member.mem_num==host }"><button type="button" class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal">수정</button></c:if>
			...
			<td>소개 ${member.mem_Introduce }</td>
				...
				<div class="w3-section w3-bottombar w3-padding-16">
					...
					<button class="w3-button w3-white w3-hide-small"
						...
						<div id="folder_add_window" class="w3-row-padding" style="position: absolute;">
							<div class="w3-col m12">
								<div class="w3-card w3-round w3-white">
									<div class="w3-container w3-padding">
										<h6 class="w3-opacity">Title</h6>
										<p>
											<input id="folder_title_text" type="text" class="w3-border w3-padding">
										</p>
										<select id="folder_secret">
											<option>공개</option>
											<option>비공개</option>
										</select>
										<input id="folder_create_btn" class="w3-button w3-theme" type="button" value="만들기" disabled="disabled" />								
									</div>
								</div>
							</div>
						</div>				
					<c:if test="${host eq member.mem_num }">
						<button id="folderAdd" class="w3-button w3-white w3-hide-small">
							<i class="fa fa-diamond w3-margin-right"></i> 
							Add List
						</button>						
					</c:if>
				</div>
				...
				<p>
					...
					<c:if test="${fol.mem_Num eq host }">					
						<c:if test="${fol.folder_Title != '기본'}">					
							<span id="delete" class="folderD" onclick="ingShoww('${fol.folder_Num}')" style="float: right;">
								Delete
							</span>
						</c:if>
					</c:if>
								...
								<table>
									...
									<c:if test="${host eq bo.mem_Num }">
										<tr colspan="2" >
											...
											<td class="boardD" onclick="ingShow('${bo.board_Num}')" style="text-align: right;"> 
												Delete
											</td>
										</tr>
									</c:if>
				...
				<div class="w3-container">
					<span
						...
						<div id="lightBoxOKk">
							...
							<input type="button" onclick="document.getElementById('myModall').style.display='none'" value="취소">
						</div>
						<p>&nbsp;</p>
				</div>
			</div>
